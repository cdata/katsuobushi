...
pub fn dispatch(args: SandboxArgs, global: Global) -> Result<()> {
    ...
    match command {
        SandboxCommand::Start {
            ...
            prompt,
            until_report,
        } => start::run(&config, agent, name, prompt, until_report, global),
        SandboxCommand::Dispatch {
            card,
            board_dir,
            force,
            until_report,
        } => dispatch::run(&config, &card, &board_dir, force, until_report, global),
        SandboxCommand::Prompt {
            instance,
            text,
            until_report,
        } => prompt::run(&config, &instance, text, until_report, global),
