...
#   # dev shell, and merge `checks` into the flake's checks output.
{
  ...
  exclude ? [ ],
  # Labels the menu commands (`format:<name>` / `lint:<name>`) and the check.
  name ? "markdown",
...
in
{
  ...
  # configuration contributes its OWN namespaced pair — `format:<name>` (rewrite
  # in place) and `lint:<name>` (read-only check) — so multiple invocations never
  # collide and there is no shared/global command. Both run from the repo root so
  # the include/exclude globs resolve workspace-relative.
  menuCommands = {
    "format:${name}" = {
